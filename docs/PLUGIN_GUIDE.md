...
新增插件：`user_plugins/cdk_rewards.py`

支持能力：

- 设置发卡管理员（支持群内 @ 成员或直接输入 QQ 号）
- 接入签到系统：
  - 可设置“首日签到奖励卡池”
  - 群内显示签到结果，CDK 私聊发放
  - 通过发放日志防止重复签到奖励、重复发送
...
- 支持邀请人数统计与奖励：
  - 可设置“邀请满 N 人 → 指定卡池”
  - 通过 `invite_records` 防止同一个被邀请用户被重复计数
- 支持查看卡池状态、邀请统计
...
- 邀请奖励依赖 OneBot `notice.group_increase` 事件中的 `operator_id` 作为邀请人。
- 如果适配器未正确提供邀请人信息，则无法精确统计邀请人数。
- 目前连续签到奖励和邀请奖励按规则去重，同一用户达到同一门槛只会发一次。
